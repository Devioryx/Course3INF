...
def rotate_clockwise(matrix):
    return [(get_column(matrix, i))[::-1] for i in range(0, len(matrix[0]))]

def rotate_counterclockwise(matrix):
    return [get_column(matrix, i) for i in reversed(range(0, len(matrix[0])))]

def flip_horizontal(matrix):
    return [matrix[i][::-1] for i in range(0, len(matrix))]

def flip_vertical(matrix):
    return [matrix[i] for i in reversed(range(0, len(matrix)))]
